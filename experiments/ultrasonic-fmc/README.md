...
Status: prospective protocol published before implementation/performance evaluation. **No pursuit pass or measured advantage yet.**
...
The protocol and notes are original project research. Future benchmark source in this directory will carry its own GPL-3.0-or-later license and applicable third-party notices, overriding the repository's MIT default for that source. No third-party binaries are bundled. The selected FINUFFT wheel contains FFTW; it is not represented as Apache-only. The input is retrieved separately under CC BY 4.0, with attribution to Alexander Velichko and Anthony Croxford and DOI 10.6084/m9.figshare.7178630.
