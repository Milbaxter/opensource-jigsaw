...
**Completed: HOLD_INSUFFICIENT_HEADROOM. All 441 native replay comparisons matched, but the conservative endpoint subset contained no non-straight routing problem. [Results and limitations](RESULTS.md). No pursuit pass.**
...
The pinned FreeCAD Adaptive2d kernel ran six fixed settings derived from one licensed upstream geometric fixture. Complete historical cleared-stock snapshots and native outcomes are captured outside the detour function’s timed body. Uninstrumented full-kernel equality and isolated request replay are required before interpreting headroom. A conservative exact geometry check and visibility graph supply a strong simple reference where needed. Prospective negative ceilings can stop unnecessary graph work.

This is a native-kernel study, not a full FreeCAD/OCC job, production shop sample, machine program, or claim of reduced machining time. Recovered route proposals may never be selected by the enclosing planner; they are not final retract savings. A positive screen only justifies designing a separate game-navigation integration experiment.

The original author protocol still says DRAFT because its bytes are preserved. The timestamped parent freeze binds that exact reviewed text, input set, source and binary hashes before execution. That freeze preceded execution. Compilation succeeded; the initial missing generated-header failure and its source-supported repair are recorded in the build metadata.
