# Attempt002 reproduction notice

The current local source is the prospectively amended canonical renderer. Read AMENDMENT-002.md and PREREGISTRATION-DRAFT.md. Do not execute before its new parent freeze. Preserve the original published root source; this snapshot belongs under attempt-002/source. Its runtime expects an isolated restored directory with the same relative layout. Raw attempt001 metadata remains under attempts/attempt-001/raw-execution. All330 reuse pins must verify. If retained files are absent, regenerate using the original frozen preparation code in a separate workspace and verify expected hashes; restore historical metadata separately. A mismatch is not an identical replay.

The prior preparation charge is574.492 seconds; effective attempt002 elapsed is this plus new wall time, bounded by10800 seconds. The prior failed attempt itself took581.824 seconds and remains disclosed separately. No held-out data have been seen.

The following installation and rights notes originate from the first freeze. The current runner replaces re-encoding with verified reuse and uses execution-attempt-002 and generated/controls-attempt-002. See the amendment for the authoritative delta.
