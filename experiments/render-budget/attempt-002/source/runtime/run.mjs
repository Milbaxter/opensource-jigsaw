...
import {reusePrepared} from './reuse.mjs';
...
await fs.mkdir(path.join(ROOT,'execution-attempt-002'),{recursive:true});
const began=Date.now();const reuseSpec=await readJSON(path.join(ROOT,'reuse-attempt-001.json'));
const priorEncodingSeconds=reuseSpec.encodingSeconds;if(!Number.isFinite(priorEncodingSeconds)||priorEncodingSeconds<=0||priorEncodingSeconds>=10800)throw Error('Invalid original preprocessing charge');
const effectiveElapsed=()=>Date.now()-began+priorEncodingSeconds*1000;
let browser,server;const children=new Set();
...
const hard=setTimeout(async()=>{await log({stage:'global-cap',seconds:effectiveElapsed()/1000,actualWallSeconds:(Date.now()-began)/1000});await cleanup();process.exit(124);},3*3600000-priorEncodingSeconds*1000);
...
try{
 await log({stage:'begin',freeze,node:process.version,argv:process.argv,priorEncodingSeconds,scope:'attempt002 canonical renderer; original279optionfiles reused with full preprocessing charge'});
 const reused=await reusePrepared(reuseSpec);await json(path.join(ROOT,'execution-attempt-002','reuse-verification.json'),reused.verified);
 ...
 const controls=await timeout(page.evaluate(c=>window.textureControls(c),cases),120000,'tiny texture controls');await json(path.join(ROOT,'execution-attempt-002','controls.json'),{freeze,browser:browser.version(),controls});
 await json(path.join(ROOT,'execution-attempt-002','prepared.json'),{...reused.prepared,reusedFromAttempt:'001',sourceFreeze:reuseSpec.source_freeze,preprocessingChargeSeconds:priorEncodingSeconds});
 const models=reused.prepared.models,cells=[];
 for(const model of models){
  const setup=await timeout(page.evaluate(m=>window.setup(m),model),120000,'original setup');await json(path.join(ROOT,'execution-attempt-002',model.name+'-setup.json'),setup);
  ...
  const bound=await byteBound(model,v=>packed(doc,model,v));await json(path.join(ROOT,'execution-attempt-002',model.name+'-byte-bound.json'),bound);
  ...
  await json(path.join(ROOT,'execution-attempt-002',model.name+'-spatialpack-frontier.json'),{rows:spatial,failure:comparatorFailure,seconds:comparatorSeconds});
  ...
  for(const budget of BUDGETS){
   ...
   cells.push(cell);await json(path.join(ROOT,'execution-attempt-002','selection-progress.json'),{freeze,cells});
  }
 }
 const seal={freeze,sealedAt:new Date().toISOString(),cells};await json(path.join(ROOT,'execution-attempt-002','selection-seal.json'),seal);const sealHash=sha(await fs.readFile(path.join(ROOT,'execution-attempt-002','selection-seal.json')));await log({stage:'seal',sha256:sealHash});
 ...
 for(const model of models){await timeout(page.evaluate(m=>window.setup(m),model),120000,'heldout setup');for(const cell of cells.filter(c=>c.model===model.name&&!c.infeasible))for(const method of ['preset','texture','greedy','cpsat','spatialpack']){
  const w=cell[method]?.winner;if(!w)continue;const bytes=await fs.readFile(path.join(ROOT,w.artifact));if(sha(bytes)!==w.sha256)throw Error('Sealed artifact changed');const r=await timeout(page.evaluate(url=>window.evaluateFile({url,heldout:true,saveImage:true}),'/'+w.artifact),120000,'heldout render');if(r.worstImage){const img=path.join(ROOT,'execution-attempt-002',`${model.name}-${cell.budget}-${method}-worst.png`);await fs.writeFile(img,Buffer.from(r.worstImage.split(',')[1],'base64'));delete r.worstImage;}outputs.push({model:model.name,budget:cell.budget,method,artifact:w.artifact,bytes:w.bytes,sha256:w.sha256,maximum:max(r.views.map(v=>v.mae)),mean:r.views.reduce((s,v)=>s+v.mae,0)/r.views.length,...r});await json(path.join(ROOT,'execution-attempt-002','heldout-progress.json'),{sealHash,outputs});
 }}
 ...
 const finalElapsed=effectiveElapsed();if(finalElapsed>=3*3600000)throw Error('Whole-run final elapsed exceeds three-hour cap');
 await json(path.join(ROOT,'execution-attempt-002','summary.json'),{freeze,sealHash,seconds:finalElapsed/1000,actualWallSeconds:(Date.now()-began)/1000,priorEncodingSeconds,withinWholeRunCap:true,decisions,technicalSupport:pass,scope:'Four authored public product assets, one pinned renderer, numerical fidelity; not customer or human quality validation'});
 if(effectiveElapsed()>=3*3600000){await json(path.join(ROOT,'execution-attempt-002','summary.json'),{freeze,sealHash,seconds:effectiveElapsed()/1000,actualWallSeconds:(Date.now()-began)/1000,priorEncodingSeconds,withinWholeRunCap:false,censored:true,technicalSupport:false,decisions});throw Error('Whole-run cap crossed during finalization');}
}catch(e){await json(path.join(ROOT,'execution-attempt-002','failure.json'),{freeze,error:e.message,stack:e.stack,seconds:effectiveElapsed()/1000,actualWallSeconds:(Date.now()-began)/1000,priorEncodingSeconds});throw e;}finally{clearTimeout(hard);await cleanup();}
